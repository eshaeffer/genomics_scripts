...
while (line != ""):
    ...
    samplenames.append(os.path.basename(line.split(".")[0]))
    line = file1.readline()
file1.close()
...
class rawVariantCounter:
    ...
    def outputRawCountTextFile(self):
        ...
        for num in range(0, len(IDs)):
            ...
            number = str(num + 1)
            data = number + "\t" + samplenames[num] + "\t" + count + "\n"
            data = data.format()
            f.write(data)
        f.close()
            ...
            while (line != ""):
                ...
                if (passedHeaders):
                    ...
                    if ("#CHROM" in line):
                        ...
                        temp = line.split()
                        sampleId = samplenames[sampleNumber]
                line = f.readline()
            sampleNumber += 1
...
class VepCounter:
    def __init__(self):
        ...
        if os.path.isdir(args.outputDir):
            ...
            outptNon = os.path.join(outputRare, "noMaf")
            ...
            if not os.path.isdir(outptNon):
                os.makedirs(outptNon)
            ...
            for filePath in files:
                ...
                outptNontmp = os.path.join(outptNon, tmp)
                ...
                self.outputPathsNon.append(outptNontmp)
    ...
    def countVarsVEP(self):
        ...
        for x in files:
            ...
            try:
                outputNon = open(pthNon[fileCount], 'w+')
            except:
                print(pthNon[fileCount] + " could not be created")
            try:
                line = f.readline()
            except:
                line = ""
            ...
            while (line != ""):
                if (passedHeaders):
                    ...
                    if ("#CHROM" in line):
                        passedHeaders = True
                        temp = line.split()
                        sampleId = samplenames[sampleCount]
                        print("Reading vepped variants of: " + sampleId)
                line = f.readline()
            sampleCount += 1
            self.sampleIds.append(sampleId)
